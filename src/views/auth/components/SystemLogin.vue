...
<template>
  <div class="flex items-center justify-center shadow rounded py-12 px-4 sm:px-6 lg:px-12">
    <div class="max-w-md w-full space-y-4">
      <div>
        <img :src="Logo" class="mx-auto h-10 w-auto" alt="System Logo" />
        <h2 class="text-center text-2xl font-bold">Sign in</h2>
        <p class="mt-2 text-center text-sm text-primary">Welcome to {{ vadmireConfigStore.name }}</p>
      </div>

      <div class="px-8 py-6 rounded-md">
        <NForm ref="signInRef" size="medium" label-width="auto" :model="signInModelData">
          <NFormItem label="Username">
            <NInput v-model:value="signInModelData.username" placeholder="Default account: admin | user" />
          </NFormItem>

          <NFormItem label="Password">
            <NInput v-model:value="signInModelData.password" placeholder="Default account admin | user" />
          </NFormItem>
          ...
          <div class="space-y-3">
            <NButton block type="primary" :loading="isSignInLoading" @click.enter="getSignInAuth">
              <template #icon>
            ...
            <NButton block secondary type="primary" @click="switchSign(false)">
              <template #icon>
                <icon-mdi:sign-in />
              </template>
              <span>Sign up</span>
