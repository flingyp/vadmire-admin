...
const { name, description, dependencies, devDependencies } = packageJson as PackageJson;
...
<template>
  <div class="space-y-2">
    <NCard :title="name" hoverable>
      {{ description }}
    </NCard>

    <NCard title="开发环境相关依赖" hoverable>
      <NDescriptions label-placement="left" bordered>
        <NDescriptionsItem v-for="(item, index) in devDependenciesList" :key="index" :label="item.name">
          {{ item.version }}
        </NDescriptionsItem>
      </NDescriptions>
    </NCard>

    <NCard title="生成环境相关依赖" hoverable>
      <NDescriptions label-placement="left" bordered>
        <NDescriptionsItem v-for="(item, index) in dependenciesList" :key="index" :label="item.name">
          {{ item.version }}
        </NDescriptionsItem>
      </NDescriptions>
    </NCard>
